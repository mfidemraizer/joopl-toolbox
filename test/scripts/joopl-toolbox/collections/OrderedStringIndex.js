...
$namespace.using("joopl", "joopl.collections", function(joopl, collections) {
	/**
		...
		@final
	*/
	collections.declareClass("OrderedStringIndex", {
		...
		members: {
			...
			*/
			...
			onAdded: function(args) {
				...
				if(this.unique) {
					if(partition.store.count(function(storeItem) {
						return storeItem[propertyName] === searchValue; 
					}) == 0) {
			...
			forEach: function(predicateFunc) {
				throw new joopl.NotImplementedException({ memberName: "OrderedStringIndex.forEach" });
			},

			reverse: function() {
				throw new joopl.NotImplementedException({ memberName: "OrderedStringIndex.reverse" });
			},
